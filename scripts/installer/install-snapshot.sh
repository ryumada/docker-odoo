...
function main() {
  ...
  if [ -f "$PATH_TO_ODOO/scripts/upload-temp-snapshots.sh" ]; then
    ln -sf "$PATH_TO_ODOO/scripts/upload-temp-snapshots.sh" "/usr/local/sbin/upload-temp-snapshots-$SERVICE_NAME" 2>/dev/null || true
  fi
